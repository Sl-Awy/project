import "../CSS/PostCard.css"
import paris from "../Img/about-Paris-2.jpg";
import rik from "../Img/3a436b9233843a9e4c4a170d3exm--dizajn-i-reklama-prevyu-avatarki-shapka-profilya.jpg";


const PostCard2 = () => {
  return (
    
      <div className="form">
      <div className="options">
        <div className="flex h-16 gap-4 items-center ">
          <div className="rounded-full w-10 h-10 flex justify-center items-center">
          <img src={rik} alt="img" style={{ borderRadius: '50%', width: '100%', height: '100%', objectFit: 'cover'  }}/>
         
          </div>
          <div className="flex flex-col">
            <span className="font-bold text-xl ">Franklin Strong</span>
            <span className="text-gray-500">23 hours ago</span>
          </div>
        </div>
        <img
          className="dots "
          src="/three-dots-vertical.svg"
          alt="ellipsis_menu"
        />
      </div>
      <p className="text">
      I visited Paris for the first time in my life.</p>
      <img
      
        className="rounded-md w-full" src={paris} alt="image" />
      <div className="flex justify-between">
        <div className="flex gap-4 items-center h-10">
          <div className="flex gap-2 items-center cursor-pointer">
            <img className="heart" src="/suit-heart-fill.svg" alt="like" />
            <span className="text-gray-50">631</span>
          </div>
          <div className="flex gap-2 items-center cursor-pointer">
            <img className="chat" src="/chat-fill.svg" alt="comment" />
            <span className="text-gray-50">56</span>
          </div>
        </div>
        <div className="flex gap-2 items-center cursor-pointer">
          <img className="share" src="/share-fill.svg" alt="share" />
          <span className="text-gray-50">12</span>
        </div>
      </div>
    </div>

   
   
  );
};

export default PostCard2;
